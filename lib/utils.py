...
def get_gcn_inp():
    # import pickle
    # inp = pickle.load(open(inp_file, 'rb'))
    inp = np.eye(5)
    return inp
...
def get_gcn_adj():
    ...
    # adj = 0.8*np.eye(5) + 0.1 * np.eye(5,5,1) + 0.1 * np.eye(5,5,-1)
    # adj[0,0]+=0.1
    # adj[-1,-1]+=0.1

    # A = np.eye(5,5,1) + np.eye(5,5,-1) +np.eye(5)
    # D=np.power(A.sum(1), -0.5)
    # D=np.diag(D)
    # adj = np.dot(np.dot(A,D).T, D)

    adj = 0.2*np.eye(5,5,1) + 0.2*np.eye(5,5,-1)
