...
__all__ = ['ResNet', 'CANet_resnet18', 'CANet_resnet34', 'CANet_resnet50', 'CANet_resnet101',
           'CANet_resnet152']
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=5, zero_init_residual=False, norm_layer=None):
        ...
        self.classifier_specific_1 = nn.Linear(512 * block.expansion, num_classes)
    ...
    def forward(self, x):
        ...
        x = self.layer4(x)
        x = self.dropout(x)
        x=self.branch_bam1(x)
        x = self.avgpool(x)
        x = x.view(x.size(0), -1)
        x = self.classifier_specific_1(x)

        return x


def CANet_resnet18(pretrained=False, **kwargs):
...
def CANet_resnet34(pretrained=False, **kwargs):
...
def CANet_resnet50(pretrained=True, **kwargs):
...
def CANet_resnet101(pretrained=False, **kwargs):
...
def CANet_resnet152(pretrained=False, **kwargs):
